...
function PendingProvidedAreaList({ navigation, route }) {
  ...
  const handleItem = (id, item) => {
    navigation.navigate("PendingProvidedAreaItem", { id: id, item: item });
  };
  ...
  const getArea = async () => {
    try {
      setFetching(true);
      let result = await getParkingArea(currentUser.user.id, "Pending");
        ...
        >
          <View style={styles.parkingnameandcars}>
            ...
            <View>
              ...
              >
                <Text style={{ color: "red", fontSize: 16 }}>Pending</Text>
              </View>
              <View
                style={[
                  { flexDirection: "row" },
...
export default PendingProvidedAreaList;
