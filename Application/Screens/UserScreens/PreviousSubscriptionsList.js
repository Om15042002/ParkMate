...
import { previousSubscriptions } from '../../Backend/Bookings/PreviousBookings';
...
function PreviousSubscriptionsList({ navigation, route }) {
    ...
    useLayoutEffect(() => {
        const getPrviousSubscriptions = async () => {
            try {
                setFetching(true)
                let searchData = { userid: currentUser.user.id, datetime: new Date() }
                var bookings = await previousSubscriptions(searchData)
        ...
        if (isFocused) {
            getPrviousSubscriptions()
        }
    }, [isFocused])

    const handleItem = (id, item) => {
        navigation.navigate("PreviousSubscriptionItem", { id: id, item: item })
    }
...
export default PreviousSubscriptionsList