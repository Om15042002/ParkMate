...
function PreviousSubscriptionItem({ route }) {
          ...
          <View style={styles.row}>
            ...
            <View style={{ justifyContent: 'center' }}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: Dimensions.get('window').width - 150 }}>
                {
                  route.params.item.days.map((day, index) => {
                    return <Text key={index} style={[{
                      marginRight: 5, color: 'grey', fontSize: 18,
                      marginBottom: 0,
                    }]}>{day}</Text>
                  })
                }
...
export default PreviousSubscriptionItem